...
SELECT genero.Genero AS NombreGenero, COUNT(*) AS CantCanciones FROM cancion
...
/*  Nota 0.00 de 0.50: Consulta incorrecta en relación a la pregunta planteada.
	Consulta correcta: 
SELECT p.`Password`, u.nombreusuario , t.TipoUsuario, p.Fecha FROM passwordxusuario p
...
/*  Nota 0.00 de 1.00: Consulta sin resolver*/
...
/* Nota 0.25 de 1.00: Consulta incorrecta en relación a la pregunta planteada. 
Consulta correcta: */

SELECT u.nombreusuario, t.TipoFormaPago, p.fechaPago FROM pagos p
INNER JOIN datospagoxusuario pxu ON pxu.idDatosPagoxUsuario = p.idDatosPagoxUsuario
INNER JOIN tipoformapago t ON t.idTipoFormaPago = pxu.idTipoFormaPago
INNER JOIN usuario u ON u.idUsuario = pxu.idusuario
WHERE t.TipoFormaPago = 'QR' AND YEAR(p.fechaPago) = 2020 GROUP BY u.idUsuario;
